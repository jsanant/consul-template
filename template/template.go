...
type Template struct {
	...
	// destination the template is rendered to
	destination string
...
type NewTemplateInput struct {
	...
	// Destination is the location on disk that the template will render  to.
	Destination string
...
func funcMap(i *funcMapInput) template.FuncMap {
	...
	r := template.FuncMap{
		...
		"pki":          pkiFunc(i.brain, i.used, i.missing, i.destination),
